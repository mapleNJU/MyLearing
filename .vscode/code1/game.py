...
class Tile_map():
    ...
    def is_map_connected(self, world_map):
        ...
        while queue:
            ...
            if (row, col) not in visited:
                ...
                if row < world_map.shape[0] - 1 and world_map[row + 1,
                                                              col] == 0:
                    queue.append((row + 1, col))
                if col > 0 and world_map[row, col - 1] == 0:
                    queue.append((row, col - 1))
                if col < world_map.shape[1] - 1 and world_map[row,
                                                              col + 1] == 0:
                    queue.append((row, col + 1))
        row, col = np.where(world_map == 0)
    ...
    def hole_generate(self):
        ...
        self.hole_generation_time_next = np.random.randint(
            self.hole_generation_time_min, self.hole_generation_time_max + 1)
...
class Agent():
    ...
    def plan_action_need(self):
        ...
        if self.step >= self.k + self.p:
            # print("yes")
            return True
        if self.reaction_strategy != 'blind_commitment' and self.env.map[
                self.target_hole[0]][self.target_hole[1]] != 1:
            # print("yes")
            return True
    ...
    def judge_new_hole_and_distance(self):
        for hole in self.env.hole:
            ...
            for found_hole in self.holes_found:
                if found_hole.x == hole.x and found_hole.y == hole.y:
                    judge = True
            if not judge and abs(hole.x - self.env.agent_position_x) + abs(
                    hole.y - self.env.agent_position_y) < abs(
                        self.env.agent_position_x - self.target_hole[0]) + abs(
                            self.env.agent_position_y - self.target_hole[1]):
                return True
        return False
    ...
    def target_select(self):
        ...
        start = Node(self.env.agent_position_x, self.env.agent_position_y,
                     None, None, 0)
        node_list = Queue()
        node_list.put(start)
        hole_list = []
        ...
        while not node_list.empty():
            ...
            for x_move, y_move in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
                ...
                if now_x >= 0 and now_y >= 0 and now_x < self.env.length and now_y < self.env.length and world_map[
                        now_x][now_y] != -2 and world_map[now_x][now_y] != -1:
                    node_list.put(
                        Node(now_x, now_y, temp_node, action_list[i],
                             temp_node.distance + 1))
