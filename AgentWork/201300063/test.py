...
def test1():
    ...
    # for gamma in [100]:
        ...
        temp_config.min_generation_time = int(temp_config.min_generation_time//gamma)+1
        temp_config.max_generation_time = int(temp_config.max_generation_time//gamma)+1
        effect_score.append(work(temp_config))
    plt.plot(gammas, effect_score, linestyle='-', marker='D', label='experiment')
    plt.grid()
    plt.legend()
    ...
    plt.plot(log10gammas, effect_score, linestyle='-', marker='D', label='experiment')
    plt.grid()
    plt.legend()
...
def test2():
    ...
    for p in [0.5, 1, 2, 4]:
        ...
        # for gamma in [1]:
            ...
            temp_config.min_generation_time = int(temp_config.min_generation_time//gamma)
            temp_config.max_generation_time = int(temp_config.max_generation_time//gamma)
            effect_score.append(work(temp_config))
        label_temp = 'p='+str(p)
        plt.plot(log10gammas, effect_score, marker=mark[i], color=color[i], label=label_temp)
        i+=1
    plt.grid()
    plt.legend()
...
def test3():
    ...
    for k in [inf, 4, 1]:
        ...
        plt.plot(log10gammas, effect_score, marker=mark[i], color=color[i], label=label_temp[i])
        i+=1
    plt.grid()
    plt.legend()
...
def test4():
    ...
    gammas = 10**log10gammas
    i=0
    for reaction_strategy in ['blind_commitment', 'notice_disappearance', 'any_new_hole', 'nearer_hole']:
        effect_score = []
        for gamma in gammas:
        # for gamma in [1]:
            temp_config = deepcopy(config)
            ...
            temp_config.max_generation_time = int(temp_config.max_generation_time//gamma)
            effect_score.append(work(temp_config))
        label_temp = ['blind commitment', 'notices target disappearance', 'target dis. or any new hole', 'target dis. or nearer hole']
        plt.plot(log10gammas, effect_score, marker=mark[i], color=color[i], label=label_temp[i])
        i+=1
    plt.grid()
    plt.legend()
    ...
    for reaction_strategy in ['blind_commitment', 'notice_disappearance', 'any_new_hole', 'nearer_hole']:
        ...
        # for gamma in [1]:
            ...
            temp_config.max_generation_time = int(temp_config.max_generation_time//gamma)
            effect_score.append(work(temp_config))
        label_temp = ['blind commitment', 'notices target disappearance', 'target dis. or nearer hole', 'target dis. or any new hole']
        plt.plot(log10gammas, effect_score, marker=mark[i], color=color[i], label=label_temp[i])
        i+=1
    plt.grid()
    plt.legend()
