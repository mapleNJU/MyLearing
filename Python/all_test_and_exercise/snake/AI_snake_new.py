...
def GameOver():
    # 设置GameOver的显示颜色
    GameOver_Surf = font.render('Your score is %s.' % len(snake_Body) - 3,
                                True, GREY)
    # 设置GameOver的位置
...
def walkable(location0, snake_body0):
    l = []
    if location0[0] + 20 < 300 and [location0[0] + 20, location0[1]
                                    ] not in snake_body0:  #+20表示下一个位置
        l.append([location0[0] + 20, location0[1]])

    if location0[0] - 20 >= 0 and [location0[0] - 20, location0[1]
                                   ] not in snake_body0:
        l.append([location0[0] - 20, location0[1]])

    if location0[1] + 20 < 300 and [location0[0], location0[1] + 20
                                    ] not in snake_body0:
        l.append([location0[0], location0[1] + 20])

    if location0[1] - 20 >= 0 and [location0[0], location0[1] - 20
                                   ] not in snake_body0:
        l.append([location0[0], location0[1] - 20])
...
def exploretheway(food_position0, snake_body0):
    ...
    while True:
        ...
        if snake_body[0][0] == food_position[0] and snake_body[0][
                1] == food_position[1]:
...
while True:
    ...
    if food_flag == 1:  ##当前布局能找到吃food_path
        virtual_tail_flag = exploretheway(
            food_Position, snake_Body)  ##虚拟蛇探路，吃到食物后如果能找到tail_path则返回1
    ...
    def drawSnake(snake_body0):
        pygame.draw.rect(DISPLAY, Purple3,
                         Rect(snake_body0[0][0], snake_body0[0][1], 20, 20))
        for j in range(1, len(snake_body0) - 1):
            pygame.draw.rect(
                DISPLAY, FORESTGREEN,
                Rect(snake_body0[j][0], snake_body0[j][1], 20, 20))
        pygame.draw.rect(DISPLAY, GREEN,
                         Rect(snake_body0[-1][0], snake_body0[-1][1], 20, 20))

    def drawFood(food_position0):
        pygame.draw.rect(DISPLAY, RED,
                         Rect(food_position0[0], food_position0[1], 20, 20))
